...
@cache
def get_sequence(grids, aim, layer, extra=0, start=None):
    # print(layer)
    # if layer >= 10:
    #     print(layer)
    # print(extra)
...
def main():
    ...
    # d_lengths = {}
    # for d in '^v<>A':
    #     d_lengths[d] = len(get_sequence(grids, d, 1, extra=0)) 

    # print(d_lengths)
    count = 0
    for code in codes: #does not consider time to get back to a
        sequence = get_sequence(grids, code, 0, extra=0)
        # print(sequence, len(sequence[0]), [len(x) for x in sequence])
        print(len(sequence))
        # s_count = sum([d_lengths[x] for x in sequence])
        s_count = len(get_sequence(grids, sequence[0], 1, extra=1))
        for s1, s2 in zip(sequence, sequence[1:]):
            s_count += len(get_sequence(grids, s2, 1, extra=1, start=s1))
